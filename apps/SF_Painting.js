...
// 初始化 WebUI 服务（新版）或旧版 WebSocket 服务
init_server();
...
export class SF_Painting extends plugin {
    ...
    async sf_setConfig(e) {
        ...
        const match = e.msg.match(/^#(sf|SF|siliconflow|硅基流动)设置(画图key|翻译key|翻译baseurl|翻译模型|生成提示词|推理步数|fish发音人|ss图片模式|ggkey|ggbaseurl|gg图片模式|上下文|ss转发消息|gg转发消息|gg搜索|ss引用原消息|gg引用原消息|ws服务|ss转发思考|群聊多人对话|ss图片上传|gg图片上传|ss必需图片|gg必需图片|ss必须返回图片|gg必须返回图片|ss群聊聊天记录条数|gg群聊聊天记录条数|webui服务|webui端口|webui密码)([\s\S]*)/)
        if (match) {
            ...
            switch (type) {
                ...
                case '群聊多人对话':
                    ...
                    break
                case 'webui服务':
                    const isWebUIEnable = value === '开';
                    if (!config_date.webUI) config_date.webUI = {};
                    config_date.webUI.enable = isWebUIEnable;
                    // 动态导入 WebUIServer
                ...
                case 'webui端口':
                    if (!config_date.webUI) config_date.webUI = {};
                    config_date.webUI.port = parseInt(value) || 8082;
                    break
                case 'webui密码':
                    if (!config_date.webUI) config_date.webUI = {};
                    if (!config_date.webUI.auth) config_date.webUI.auth = {};
                    config_date.webUI.auth.password = value;
                    break
                default:
                    return
            }
...
有效期: 5分钟,`
        ...
        // 如果存在外网地址，优先展示外网（适合远程访问）
        if (addrs.wan) {
            forwardMsg.push(`🌍 【外网访问】
一键登录: ${makeQuickUrl(addrs.wan)}
手动访问: ${addrs.wan}`)
        }

        // 如果存在内网地址，展示内网（适合在同一个路由器下访问）
        if (addrs.lan) {
...
手动访问: ${addrs.ipv6}`)
        ...
        if (!addrs.wan && !addrs.lan) {
            forwardMsg.push(`💻 【本机访问】
一键登录: ${makeQuickUrl(addrs.local)}
手动访问: ${addrs.local}`)
        }

        // 拼接通用的操作提示
        forwardMsg.push(`👉 手动登录说明:
输入 QQ: ${userQQ}
输入验证码: ${code}`)

        e.reply(await common.makeForwardMsg(e, forwardMsg, e.msg));
    }
    ...
    async sf_draw(e) {
        ...
        if (config_date.imageModel.match(/stabilityai\/stable-diffusion-3-medium|stabilityai\/stable-diffusion-xl-base-1\.0|stabilityai\/stable-diffusion-2-1|stabilityai\/stable-diffusion-3-5-large|Qwen\/Qwen-Image-Edit|Kwai-Kolors\/Kolors/)) {
            canImg2Img = true;
        }
    ...
    async txt2img_generatePrompt(e, userPrompt, config_date) {
        ...
        if (e.sfRuntime.isgeneratePrompt ?? config_date.generatePrompt) {
            if (!onleReplyOnce && config_date.replyStartMsg) {
                e.reply(`@${e.sender.card || e.sender.nickname} ${e.user_id}正在为您生成提示词并绘图...`, true, { recallMsg: 60 })
                onleReplyOnce++
            }
        ...
        if (!onleReplyOnce && config_date.replyStartMsg) {
            e.reply(`@${e.sender.card || e.sender.nickname} ${e.user_id}正在为您生成图片...`, true, { recallMsg: 60 })
            onleReplyOnce++
        }
        return finalPrompt;
    }
    ...
    async sf_chat(e, config_date = undefined) {
        ...
        if (usingApiIndex > 0 && config_date.ss_APIList && config_date.ss_APIList[usingApiIndex - 1]) {
            ...
            mustReturnImgRetriesTimes = config_date.ss_mustReturnImgRetriesTimes
            groupContextLength = config_date.ss_groupContextLength
            quoteMessage = config_date.ss_quoteMessage || true
            forwardThinking = config_date.ss_forwardThinking
            enableImageUpload = config_date.ss_enableImageUpload
        }
        ...
        // 处理引用消息,获取图片和文本
        mustNeedImgLength = Math.min(Math.max(0, parseInt(mustNeedImgLength) || 0), maxCollectedImages)
        ...
        if (mustNeedImgLength > 0 && defaultImages && defaultImages.length > 0) {
            ...
            for (const rawPath of defaultImages) {
                if (e.img.length >= maxCollectedImages) break;
                try {
        ...
        if (paintModel && config_date.replyStartMsg)
        ...
        historyImages = historyImages.slice(0, Math.max(0, maxCollectedImages - currentImages.length))
...
${e.sfRuntime.isgeneratePrompt === undefined ? "Tags中可用：--自动提示词[开|关]" : ""}`
                ...
                if (e.ws) {
                    ...
                    if (config_date.simpleMode) {
                        const msgx = await common.makeForwardMsg(e, [str_1, { ...segment.image(imageUrl), origin: true }, str_2], `${e.sender.card || e.sender.nickname} 的${canImg2Img ? "图生图" : "文生图"}`)
                        e.reply(msgx)
                    } else {
                        const msgx = await common.makeForwardMsg(e, [str_1, str_2], `${e.sender.card || e.sender.nickname} 的${canImg2Img ? "图生图" : "文生图"}`)
                        e.reply(msgx)
                        e.reply({ ...segment.image(imageUrl), origin: true }, config_date.sendImgQuote_Message)
                    }
                }
    ...
    async gg_chat(e, config_date = undefined) {
        ...
        if (usingApiIndex > 0 && config_date.gg_APIList && config_date.gg_APIList[usingApiIndex - 1]) {
            ...
            // 只有当APIList中的字段有值时才使用该值
            ggBaseUrl = apiConfig.apiBaseUrl || config_date.ggBaseUrl || "https://sfgemini.vledx.ggff.net" || "https://a.geminiproxy.ggff.net"
            ...
            forwardMessage = (typeof apiConfig.forwardMessage !== 'undefined') ? apiConfig.forwardMessage : false
            mustNeedImgLength = (typeof apiConfig.mustNeedImgLength !== 'undefined') ? apiConfig.mustNeedImgLength : 0
            defaultImages = (Array.isArray(apiConfig.defaultImages) && apiConfig.defaultImages.length > 0) ? apiConfig.defaultImages : (config_date.gg_defaultImages || [])
            mustReturnImgRetriesTimes = (typeof apiConfig.mustReturnImgRetriesTimes !== 'undefined') ? apiConfig.mustReturnImgRetriesTimes : 0
            paintModel = (typeof apiConfig.paintModel !== 'undefined') ? apiConfig.paintModel : false
            groupContextLength = (typeof apiConfig.groupContextLength !== 'undefined') ? apiConfig.groupContextLength : 0
            quoteMessage = (typeof apiConfig.quoteMessage !== 'undefined') ? apiConfig.quoteMessage : false
            ...
            // 使用默认配置
            ggBaseUrl = config_date.ggBaseUrl || "https://sfgemini.vledx.ggff.net" || "https://a.geminiproxy.ggff.net"
            ...
            model = config_date.gg_model || "gemini-2.5-flash"
            ...
            defaultImages = config_date.gg_defaultImages || []
            mustReturnImgRetriesTimes = config_date.gg_mustReturnImgRetriesTimes
            groupContextLength = config_date.gg_groupContextLength
            quoteMessage = config_date.gg_quoteMessage || true
            useSearch = config_date.gg_useSearch || true
        ...
        if (mustNeedImgLength > 0 && defaultImages && defaultImages.length > 0) {
            ...
            for (const rawPath of defaultImages) {
                ...
                try {
                    ...
                    if (!filePath) continue;
                    if (!fs.existsSync(filePath)) {
                        logger.warn(`[SF插件][gg]默认图片文件不存在: ${filePath}`);
                        continue;
                    }
                    const ext = filePath.slice(filePath.lastIndexOf('.')).toLowerCase();
                    const mimeType = extMimeMap[ext] || 'image/jpeg';
                    const buffer = fs.readFileSync(filePath);
                    e.img.push(`data:${mimeType};base64,${buffer.toString('base64')}`);
                    logger.info(`[SF插件][gg]已加载默认图片: ${filePath}`);
                } catch (err) {
                    logger.error(`[SF插件][gg]读取默认图片失败: ${rawPath}`, err);
                }
            }
        }
        ...
        if (isVideo && targetUrl) {
            ...
            try {
                ...
                if (typeof targetUrl === 'string' && (targetUrl.startsWith('data:') || targetUrl.startsWith('base64://'))) {
                    ...
                    const estimatedSizeBytes = (base64Data.length * 0.75) - padding;
                ...
                logger.error(`[派蒙nai][gg]获取视频时出错: ${error.message}`);
            ...
            for (const imgUrl of e.img.slice(0, maxCollectedImages)) {
                try {
                    ...
                    const contentLength = response.headers.get('content-length');
                    if (contentLength && parseInt(contentLength) > maxSizeBytes) {
                        logger.warn(`[派蒙nai][gg]图片大小超过限制 (${maxSizeMB}MB)，跳过: ${imgUrl}`);
                        continue;
                    }
...
async function init_server() {
    ...
    // 优先使用新版 WebUI 服务
    if (config.webUI?.enable) {
        logger.mark('[sf插件] 检测到新版 WebUI 配置，启动 WebUI 服务...');
        await WebUIServer.start();
        return;
    }
    ...
    // 兼容旧版 WebSocket 服务
    if (config.enableWS) {
        logger.mark('[sf插件] 检测到旧版 WebSocket 配置，启动兼容模式...');
        await init_legacy_server(config);
    }
}

/**
 * 旧版 WebSocket 服务初始化（向后兼容）
 */
async function init_legacy_server(config) {
    // 使用全局对象存储服务器实例
    if (!Ws_Server.sfPluginServer) {
        // 创建HTTP服务器
